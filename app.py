...
def get_ice_servers():
    ...
    try:
        ...
        print(
            f"[TURN] Twilio ICE 서버 발급 성공: "
            f"{len(token.ice_servers)}개"
        )
        ...
        print(f"[TURN] Twilio 발급 실패, STUN으로 폴백: {error_msg}")

        # Twilio 자격증명이 없거나 발급 실패 시 STUN만으로 폴백
        # (일부 네트워크 환경에서는 연결이 안 될 수 있음)
        return (
