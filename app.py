...
# ============================================================
# 개인 기준선(baseline) 저장/조회
#
# PoC 단계 임시 저장소: 서버 로컬 JSON 파일 하나에 이름(식별자)별로
# 첫 측정치를 저장해두고, 다음 측정 때 그 값과 비교한다.
...
def _load_baseline_store():
    ...
    try:
        with open(BASELINE_STORE_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {}
...
def save_baseline(user_id, metrics, risk_score):
...
def compute_risk_score(metrics, reaction_ms=None):

    # NOTE: 아래 가중치·환산식은 임상적으로 검증된 값이 아닌
    # 프로토타입용 임의 스케일링이다. "이런 신호를 본다"는 방향성은
    # 관련 논문들(눈깜빡임률-MCI, 시선패턴-AD, 안면비대칭-AD 등)과
    # 궤를 같이하지만, 구체적 점수화 공식 자체는 검증되지 않았다.

    raw_scores = {
        "표정 변화": metrics["expression_change"] * 0.6,
        "미세 근육 움직임": metrics["micro_movement"] * 0.8,
        "안면 비대칭": metrics["facial_asymmetry"] * 0.5,
        "눈 깜빡임 빈도(회/분)": metrics["blink_rate"],
    }

    if metrics["gaze_variability"] is not None:
        raw_scores["시선 이동 변동성"] = metrics["gaze_variability"] * 0.4
    else:
        raw_scores["시선 이동 변동성"] = None

    if reaction_ms is not None:
        # 반응속도는 빠를수록(값이 작을수록) 좋으므로, 300ms를
        # 기준선으로 두고 그보다 느린 정도를 점수화한다.
        raw_scores["반응속도(ms)"] = max(0.0, (reaction_ms - 300.0) * 0.05)
    else:
...
# ============================================================
    ...
    if screening is None:
        ...
        st.caption(
            "※ 각 지표를 살펴보는 방향은 관련 연구(눈깜빡임률-MCI, "
            "시선패턴-AD, 안면비대칭-AD 등)와 궤를 같이 하지만, "
            "이 화면의 구체적 점수·가중치는 임상적으로 검증되지 "
            "않은 프로토타입 계산값이며 의학적 진단 근거로 사용할 "
            "수 없습니다."
        )
        ...
        for name, val in raw_scores.items():
            ...
            if val is None:
                st.write(f"- {name}: 측정 안 됨")
            else:
                st.write(f"- {name}: {val:.2f} pts")
        ...
        if user_id:
            ...
            if baseline is None:
                ...
                if st.button("이번 결과를 내 기준선으로 저장", use_container_width=True):
                    ...
                    save_baseline(user_id, screening["metrics"], risk_score)
                ...
                st.write(
                    ...
                    f"(당시 위험도 스코어: {baseline['risk_score']:.1f})"
                )
