...
class QuantumOptimizedScore:
    ...
    mode: str = "data_driven"      # "data_driven" 또는 "bootstrap" — 어떤 공식으로 계산됐는지


# 부스트 여부(0/1)에 따라 기본 가중치에 곱해지는 배율.
...
def compute_quantum_optimized_score(
    ...
    mode: str = "data_driven",
) -> QuantumOptimizedScore:
    """6개 지표를 전부 사용하되(v3 학습모델과 동일하게 6개 전부가 항상
    결합에 참여), QAOA가 그중 어느 지표의 가중치를 부스트할지를 정한다.

    [2026-09-15 수정] mode 매개변수 추가 — data_driven과 bootstrap은
    입력값의 통계적 성격 자체가 다르다(전자는 표준화된 z-score, 0~4
    범위. 후자는 고정 계수×클리핑만 거친 원시값, 0~15 범위). 이 구분
    없이 항상 data_driven용 지수 스쿼싱 공식을 쓰면, bootstrap의
    0~15 스케일 값이 거의 항상 100 근처로 과포화된다(실제로 AI=7.7점,
    양자=97.8점처럼 벌어지는 버그가 있었다 — 실측으로 발견하고 고침).

      data_driven: 기본 가중치 = METRIC_WEIGHTS[m] (문헌 기반),
                   combined_Z = 가중평균 → 지수 스쿼싱
                   (legacy_scoring.data_driven_score와 동일 공식)
      bootstrap:   기본 가중치 = 1.0 (전부 동일 — bootstrap_score()가
                   원래 개별 가중치 없이 단순평균하던 것과 동일하게 맞춤),
                   combined_raw = 가중평균 → ×1.2, 0~100 클립
                   (legacy_scoring.bootstrap_score와 동일 공식)

    이렇게 해야 '양자 경로가 부스트 대상만 다르게 고른, 같은 방식의
    AI 계산'이 되어 두 점수가 같은 척도 위에서 비교 가능해진다.
    """
    ...
    if mode == "bootstrap":
        base_weights = {m: 1.0 for m in METRIC_NAMES}
    else:
        base_weights = METRIC_WEIGHTS
