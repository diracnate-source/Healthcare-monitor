...
def compute_dual_path_result(
    ...
    if z_plus_by_metric:
        ...
        # [버그 수정 2026-09-14] quantum_optimizer.compute_quantum_optimized_score()는
        # mode 인자를 받지 않는다 (z_plus_by_metric 자체가 data_driven 모드에서만
        # 채워지므로, 이 함수가 호출되는 시점엔 이미 data_driven으로 확정돼 있다).
        # 예전 코드가 존재하지 않는 mode= 키워드를 넘겨 TypeError로 양자 경로 전체가
        # 죽는 문제가 있었다 — 아래처럼 제거한다. legacy_mode 자체는 quantum_result
        # 딕셔너리에 정보로 남겨, 이 세션이 어떤 모드에서 계산됐는지는 계속 기록한다.
        legacy_mode = detail.get("scored_by", "data_driven")
