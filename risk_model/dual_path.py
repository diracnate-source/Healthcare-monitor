...
def compute_dual_path_result(
    ...
    if z_plus_by_metric:
        ...
        # [2026-09-15 재수정] 지난 수정에서 mode= 인자를 완전히 제거했으나,
        # 그건 진짜 원인을 잘못 짚은 임시방편이었다. 진짜 원인은
        # quantum_optimizer.compute_quantum_optimized_score()가 mode
        # 인자 자체를 아직 안 받고 있던 것이었다 — 이번에 그 함수에
        # mode 매개변수를 정식으로 추가했으므로(quantum_optimizer.py
        # 참고), 여기서도 legacy_mode를 다시 정확히 전달한다.
        #
        # detail["legacy_mode"]를 쓰는 이유: detail["scored_by"]는
        # "learned_model"일 수도 있는데, 그 경우에도 z_plus_by_metric
        # 자체는 legacy(bootstrap 또는 data_driven) 계산에서 나온
        # 값이므로, 어떤 스케일인지는 scored_by가 아니라 legacy_mode가
        # 정확히 알려준다.
        legacy_mode = detail.get("legacy_mode", "data_driven")
        if legacy_mode not in ("bootstrap", "data_driven"):
            legacy_mode = "data_driven"
        ...
        quantum_result = {
            ...
            "mode": legacy_mode,
            "is_statistically_calibrated": legacy_mode == "data_driven",
        }
