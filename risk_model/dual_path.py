...
def compute_dual_path_result(
    ...
    if z_plus_by_metric:
        ...
        # legacy_mode(bootstrap/data_driven)에 맞는 결합공식을 쓰도록
        # 그대로 전달한다 — 두 모드는 입력값의 스케일 자체가 달라서
        # (z-score vs 클리핑된 원시값), 잘못 섞으면 점수가 왜곡된다
        # (실제로 발견하고 고친 문제, quantum_optimizer.py 주석 참고).
