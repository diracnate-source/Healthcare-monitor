...
def convert_app_personal_store(
    ...
    - [개선] m2 = entry["metric_m2"][지표]가 있으면 그 값(없으면 0.0).
      ...
      baseline_store.json과도 안전하게 호환된다.
    """
    result: Dict[str, PersonalStats] = {}
    if not user_id:
        for name in METRIC_NAMES:
            result[name] = PersonalStats(n_personal=0, mean=0.0, m2=0.0)
        return result
...
def compute_risk_score_v3(
    ...
    enable_quantum_modifier: bool = False,
):
    ...
    enable_quantum_modifier=True로 넘기면, legacy 경로가 데이터기반
    모드(data_driven)로 실제 최종 점수를 산출한 경우에 한해, 실험적
    양자회로 보정(quantum_modifier.py)을 추가로 적용한다. 기본값은
    False이며, 이 옵션은 임상적 근거가 없는 실험적 기능이다 — 상세
    설명은 risk_model/quantum_modifier.py 모듈 docstring 참고.
    """
    ...
    if model is not None and model.is_reliable():
        ...
        mode = "learned_model"
        combined_z = None
    elif enable_quantum_modifier and legacy_result["mode"] == "data_driven":
        # 실험적 양자회로 보정 — legacy_scoring이 이미 계산한 6개 z+ 값을
        # 그대로 재사용하며, 여기서 근거 없는 신규 지표를 만들지 않는다.
        from .quantum_modifier import apply_quantum_modifier

        adjusted = apply_quantum_modifier(legacy_result, enabled=True)
        risk_score = adjusted["risk_score"]
        tier = adjusted["tier"]
        mode = "data_driven"
        combined_z = legacy_result.get("combined_z")
        quantum_info = adjusted["quantum_modifier"]
    else:
    ...
    # [신규] 불확실성(신뢰구간) 계산 — legacy(v2) 경로의 point estimate
    # 기준으로 계산한다. learned_model/quantum 경로가 최종 표시 점수로
    # 쓰이는 경우에도, '이 세션의 통계적 불확실성이 어느 정도인지' 감을
    # 잡는 참고용으로 legacy 기준 신뢰구간을 함께 제공한다.
    from .uncertainty import compute_risk_score_ci

    ci_result = compute_risk_score_ci(
        raw_values, population_stats, personal_stats,
        point_estimate=legacy_result["risk_score"], tier=legacy_result["tier"],
        mode=legacy_result["mode"],
    )
    ...
    detail = {
        ...
        "legacy_mode": legacy_result["mode"],  # "bootstrap" 또는 "data_driven" — scored_by가
                                                 # "learned_model"이어도 이 필드로 원본 legacy
                                                 # 계산이 어떤 모드였는지 항상 알 수 있다.
        "scored_by": mode,  # "bootstrap" / "data_driven" / "learned_model"
        "clipped_metrics": legacy_result.get("clipped_metrics", []),
        "quantum_modifier": quantum_info,  # None이면 미적용(기본값)
        "confidence_interval": {
            "available": ci_result.ci_available,
            "lower": ci_result.ci_lower,
            "upper": ci_result.ci_upper,
            "note": ci_result.note,
        },
        # [2026-09-15 수정] 이제 부트스트랩 모드에서도 채워진다 —
        # data_driven은 실제 z-score(표준화된 값), bootstrap은 클리핑된
        # 원시값(raw_clipped)이다. 두 값의 통계적 성격이 다르므로, 이
        # 값을 소비하는 쪽(quantum_optimizer)은 반드시 legacy_mode를
        # 같이 넘겨 어떤 공식을 쓸지 구분해야 한다 (dual_path.py 참고).
        "z_plus_by_metric": _extract_elevation_values(legacy_result),
    }

    return risk_score, raw_scores, detail


def _extract_elevation_values(legacy_result: dict) -> Dict[str, float]:
    """legacy_result(bootstrap 또는 data_driven)에서 '이 지표가 평소보다
    얼마나 튀었는지'를 나타내는 값들을 모드에 맞게 꺼낸다. 양자 경로
    (quantum_optimizer)는 이 값의 통계적 출처를 몰라도 동작하지만,
    두 모드의 값은 스케일이 다르므로 섞어 비교하면 안 된다 — 반드시
    함께 반환되는 legacy_mode를 quantum_optimizer.compute_quantum_optimized_score()의
    mode 인자로 전달해야 한다.
    """
    detail = legacy_result.get("detail", {})
    mode = legacy_result.get("mode")

    if mode == "data_driven":
        return {name: entry["z_plus"] for name, entry in detail.items()}
    if mode == "bootstrap":
        return {name: entry["raw_clipped"] for name, entry in detail.items()}
    return {}
