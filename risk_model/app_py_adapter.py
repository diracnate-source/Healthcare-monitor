...
def compute_risk_score_v3(
    ...
    detail = {
        ...
        # 데이터기반 모드일 때만 채워짐. 양자 회로 시각화(quantum_modifier.py의
        # compute_entanglement_analysis 등)에 그대로 넘겨 재사용할 수 있도록
        # z+ 값들을 노출한다. 부트스트랩 모드에서는 빈 dict.
        "z_plus_by_metric": {

            name: entry["z_plus"]
            for name, entry in legacy_result.get("detail", {}).items()
        } if legacy_result["mode"] == "data_driven" else {},
    }
