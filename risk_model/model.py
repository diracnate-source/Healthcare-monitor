...
class RiskModel:
    ...
    def __init__(self) -> None:
        ...
        # [개선] 사람이 명시적으로 승인하기 전까지는 절대 True가 되지
        # 않는 필드. train.py는 이 값을 절대 True로 설정하지 않는다 —
        # approve_model.py를 통해서만, 사람이 training_report.json을
        # 검토한 뒤 수동으로 승인해야 채워진다.
    ...
    def is_reliable(self) -> bool:
        ...
        [개선] 기존에는 통계적 조건 두 가지(충분한 샘플 수 + legacy 대비
        검증된 성능 우위)만 만족하면 자동으로 True가 되어, 검증을
        통과하는 즉시 스스로 프로덕션에 반영될 수 있었다. 의료 목적
        시스템에서는 알고리즘이 사전 승인 없이 스스로 바뀌는 것 자체가
        위험하므로(규제 관점에서도 문제), 통계적 조건에 더해
        is_approved(사람의 명시적 승인)가 반드시 True여야 하도록
        게이트를 추가했다. is_approved는 이 클래스 내부 어디서도
        자동으로 설정되지 않는다 — approve_model.py를 통해서만
        사람이 직접 설정한다.
        """
        return (
    ...
    def load(cls, path: str | Path) -> "RiskModel":
        ...
        # 하위 호환: is_approved 필드가 없는 옛 모델 폴더는 미승인으로 간주
