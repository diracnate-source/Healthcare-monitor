...
def approve_model(model_dir: str, approver: str, force: bool = False) -> bool:
    ...
    model_type = meta.get("model_type", "risk_model_v3")
    ...
    if model_type == "fusion_v4":
        print(f"v3 단독 대비 통계적 우위 확인됨: {meta.get('is_validated_better_than_v3')}")
    ...
    if model_type == "fusion_v4" and not meta.get("is_validated_better_than_v3", False):
        print(
            "[승인 불가] 이 Fusion 모델은 legacy보다는 낫지만, v3(학습모델) 단독 대비 "
            "유의미한 개선이 확인되지 않았습니다 (is_validated_better_than_v3=False). "
            "Fusion은 v3보다도 나아야 승인 대상입니다 — 승인해도 FusionModel.is_reliable()이 "
            "계속 False를 반환하므로 실질적으로 승인의 의미가 없습니다. v3 단독을 계속 "
            "사용하는 것을 권장합니다."
        )
        return False
