...
MIN_POPULATION_N = 1
# [2026-09-14, 테스트/데모용으로 하향 조정] 원래 값은 30이었다. 표본이
# 쌓이기 전까지는 항상 부트스트랩 모드라 z-score 자체가 없어 양자
# 경로(QAOA)가 계산되지 않는데, 테스트 단계에서 양자 경로 동작을
# 최대한 빨리 확인하고 싶어 1로 낮췄다.
#
# 주의 1: 이 값이 작을수록 모집단 평균/표준편차 추정이 통계적으로
# 극도로 불안정해진다. 표본 1~2개로 추정한 표준편차는 사실상 의미가
# 없다. 실사용자 데이터가 쌓이기 시작하면 원래 값(30) 또는 더 큰 값
# 으로 되돌리는 것을 강력히 권장한다.
#
# 주의 2: MIN_POPULATION_N=1이면 모집단 n=1 시점부터 데이터기반 모드
# 진입을 시도하는데, 그 시점엔 분산(표본 2개 이상 필요)이 아직
# 정의되지 않아 지표별 std가 전부 None이다. 원래 코드는 이 경우
# data_driven_score()가 ValueError를 그대로 던져 앱이 죽었다 — 아래
# compute_legacy_risk()에 방어 코드를 추가해, 이 경우 조용히
# bootstrap_score()로 폴백하도록 고쳤다(표본이 더 쌓이면 자동으로
# 데이터기반 모드로 넘어간다).
EMPIRICAL_BAYES_K = 4
SQUASH_LAMBDA = 2.0
...
def compute_legacy_risk(
    ...
    try:
        ...
        # [2026-09-14 방어 코드] MIN_POPULATION_N을 낮게 설정한 경우
        # (예: 테스트용 1), 게이트(blink_rate.n)는 통과했지만 다른
        # 지표들의 분산이 아직 정의되지 않아(표본 2개 미만) 계산 가능한
        # 지표가 하나도 없을 수 있다. 이 경우 앱을 죽이는 대신 조용히
        # 부트스트랩으로 폴백한다 — 표본이 조금만 더 쌓이면 다음 세션
        # 부터는 자동으로 데이터기반 모드가 정상 동작한다.
        return bootstrap_score(metrics)
