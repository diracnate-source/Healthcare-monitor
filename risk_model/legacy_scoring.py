...
MIN_POPULATION_N = 30
# [2026-09-15 원복] 이전 세션에서 "양자 경로가 첫 검사부터 안 보인다"는
# 문제를 이 값을 1로 낮춰서 우회했었다. 이건 잘못된 해법이었다 — 표본
# 1~2개로 추정한 모집단 평균/표준편차는 통계적으로 의미가 없고, 그
# 상태에서 z-score를 계산하는 건 숫자를 지어내는 것과 다르지 않다.
# 진짜 원인은 따로 있었다: bootstrap 모드(표본 부족 시 폴백)가 애초에
# 양자 경로(QAOA)에 넘겨줄 값 자체를 만들지 않고 있었다. 그래서
# MIN_POPULATION_N을 낮추는 대신, bootstrap_score()가 자체적으로 이미
# 계산하고 있던 '클리핑된 원시값'을 detail로 노출해 양자 경로가 그
# 값을 쓰도록 고쳤다(아래 bootstrap_score() 참고). 이러면 표본이
# 하나도 없는 첫 검사부터 양자 경로가 정상적으로 계산되면서도,
# data_driven 모드의 통계적 엄밀성(표본 30개 이상)은 그대로 지켜진다.
EMPIRICAL_BAYES_K = 4
SQUASH_LAMBDA = 2.0
...
def bootstrap_score(metrics: Dict[str, Optional[float]]) -> Dict:
    ...
    return {
        ...
        # [2026-09-15 추가] data_driven 모드의 detail(z_plus)과 형태를
        # 맞춰, 양자 경로(QAOA)가 부트스트랩 모드(=표본이 아직 없는
        # 첫 검사부터)에서도 계산될 수 있게 한다.
        #
        # 주의: 이 값은 모집단 대비 표준화된 z-score가 아니라, 고정
        # 계수를 곱하고 상한선으로 클리핑한 '임시 원시값'이다.
        # data_driven 모드의 z-score(보통 0~4 범위)와 스케일이 다르므로
        # (여긴 0~15 범위), 이 값을 소비하는 쪽(quantum_optimizer.py)은
        # 반드시 mode="bootstrap"을 넘겨 그에 맞는 결합공식(선형 스케일)을
        # 쓰도록 해야 한다 — data_driven용 지수 스쿼싱 공식을 그대로
        # 적용하면 점수가 100 근처로 과포화되는 버그가 실제로 있었다.
        "detail": {
            name: {"raw_clipped": v} for name, v in clipped.items() if v is not None
        },
    }
...
def compute_legacy_risk(
    ...
    try:
        ...
        # [방어 코드, 유지] MIN_POPULATION_N=30으로 정상화된 뒤에도,
        # 극히 드문 경우(예: 특정 지표만 분산이 0에 매우 가까운 경우)
        # 계산 가능한 지표가 하나도 없을 수 있다. 이 경우 앱을 죽이는
        # 대신 조용히 부트스트랩으로 폴백한다 — 순수 안전망이며, 정상
        # 운영에서는 거의 발동하지 않아야 한다.
        return bootstrap_score(metrics)
